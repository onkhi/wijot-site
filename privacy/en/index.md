# wijot Privacy Policy

Effective date: April 1, 2026

## 1. Scope

This Privacy Policy applies to the iOS app "wijot" and its related support pages.

## 2. Information wijot handles

### 2.1 Memo content and settings

wijot is a local-first app.  
In the current implementation, memo content and certain settings are stored primarily on the user's device or in the shared storage area used for widgets.

### 2.2 Purchase status information

wijot uses StoreKit to check the status of subscriptions and lifetime purchases.  
Billing, payment methods, and other payment details are handled by Apple.

### 2.3 Information you provide when contacting support

If you contact support, we may receive the information you choose to provide, such as your name, email address, message content, and attachments.

## 3. Purposes of use

We use the information above for the following purposes.

- To provide and maintain app features
- To keep widget presentation in sync with app data
- To reflect purchase status, restore purchases, and show plan status
- To respond to support inquiries
- To investigate issues and improve support quality

## 4. Third-party services

### 4.1 Apple

Apple is involved in App Store distribution, in-app purchases, subscription management, and purchase restoration.  
The handling of related information is also subject to Apple's terms and privacy policies.

### 4.2 Analytics and advertising SDKs

At this time, the current implementation does not include custom analytics SDKs or advertising SDKs.  
If this changes in the future, this policy will be updated.

## 5. External transmission and storage

At this time, the current implementation does not include a custom feature that syncs or uploads memo content to servers operated by us.  
However, information may be processed through support channels you use or through Apple's purchase infrastructure when applicable.

## 6. Your choices

You may manage the following as needed.

- Delete memo content in the app
- Manage subscriptions through the App Store
- Stop contacting support

Because local data can be lost due to device failure, app deletion, or missing backups, please manage your own backups and retention as needed.

## 7. Changes

We may update this policy due to legal requirements, feature changes, or operational needs.  
If a material change occurs, we will provide notice through an appropriate method.

## 8. Contact

- Operator name: `Kai Onishi`
- Email address: `bigwestopen@gmail.com`
- Website: `https://onkhi.github.io/wijot-site/`
